...
//motor spin
//state 1
void spin(int dir,int speed,int step){
	if(dir == 1){
		//clockwise spin
...
int RecieveInput(void){
	while(1){
		if(((GPIO_PORTJ_DATA_R^0b10)&0b10)==0b10) //if PJ1 is pushed program start
			return 2; 
		if(((GPIO_PORTJ_DATA_R^0b1)&0b1)==0b1) //if PJ0 is pushed program end
			return 1; 
	}
...
void waitForPython(void){
	while(1){
		...
		if (input == 0x31)	{		//ASCII for '1' is 0x31 or decimal 49
			FlashAllLEDs();
			return;				//python is start, can now start onborad program
		}
	}
}
...
int main(void) {
  ...
  uint8_t dataReady;
	...
	int numtest = 64; //for a whole rotation
...
//Measurement part
	while(1){
		//get input, if start then start, if end break
		...
		//      sprintf(printf_buffer,"%u, %u, %u, %u, %u\r\n", RangeStatus, Distance, SignalRate, AmbientRate,SpadNum);
				sprintf(printf_buffer,"%u\n", Distance);
					UART_printf(printf_buffer);
				spin(1,5,512/numtest);//full rotation is 512 step, so each test is 512/number of test
			}
			
			spin(-1,5,512);//spin 1 rotation back;
		
			
	}
	//turn off the sensor and motor to save energy
	VL53L1X_StopRanging(dev);
	GPIO_PORTH_DATA_R = 0b0000;
